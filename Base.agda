...
infixr 1 _≃⟨_⟩_
_≃⟨_⟩_ : ∀ {ℓ ℓ' ℓ''} {B : Set ℓ'} {C : Set ℓ''} (A : Set ℓ) → A ≃ B → B ≃ C → A ≃ C
A ≃⟨ α ⟩ β = α ▪≃ β
...
Σ[≃]→≃ : ∀ {ℓ ℓ' ℓ''} {A : Set ℓ} {P : A → Set ℓ'} {Q : A → Set ℓ''}
       → ((a : A) → P a ≃ Q a) → Σ A P ≃ Σ A Q
Σ[≃]→≃ {A = A} {P} {Q} eq = f , (qinv→isequiv (g , ε , η))
  where
  f : Σ A P → Σ A Q
  f (a , u) = a , ≃→ (eq a) u
  ...
  ε : f ∘ g ~ id
  ε (a , u) = pairΣ≡ (refl a , ≃ε (eq a) u)

  η : g ∘ f ~ id
  η (a , u) = pairΣ≡ (refl a , ≃η (eq a) u)
